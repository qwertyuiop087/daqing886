...
class RedPacketManager:
    ...
    async def send_code(self, phone: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            return True, "✅ 验证码已发送，请查看 Telegram 消息"
        except PhoneNumberInvalid:
            return False, "❌ 手机号无效"
        except FloodWait as e:
            return False, f"❌ 操作频繁，请等待 {e.value} 秒"
...
async def add_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not phone.startswith('+'):
        await update.message.reply_text("❌ 格式错误，请以+开头（如 +86138xxxx）")
        return PHONE
    ...
    # 发送验证码（修复关键）
    ok, msg = await manager.send_code(phone)
