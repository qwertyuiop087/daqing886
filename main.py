...
def ins_done(m):
    ...
    safe_send_msg(cid,f"✅总行数：{total}，正在处理插雷...")
...
def split_send_clean(cid,uid,lines,name):
    ...
    if u['balance'] < fee:
        ...
        return
    safe_send_msg(cid,f"✅总行数：{total}，正在生成纯净分包...")
    chunks = [lines[i:i+u['line']] for i in range(0,total,u['line'])]
    media = []
    idx = 1
    batch = 1
...
def callback_handler(call):
    ...
    if data == "mode":
        ...
        bot.send_message(cid, "🗑️输入要作废的卡密")
...
def heb(m):
    ...
    if u['balance'] < fee:
        ...
        return
    safe_send_msg(cid, f"✅正在合并，总行数：{total}行")
    # 生成文件
    if u['mode']=="VCF":
...
def doc(m):
    ...
    try:
        ...
        safe_send_msg(cid, "📥正在解析文件，请耐心等待...")
